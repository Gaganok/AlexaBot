...
public class AvailableIDRequestHandler implements RequestHandler {

    @Override
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName("ChatbotAvailableIDIntent"));
    }
    ...
    public Optional<Response> handle(HandlerInput input) {
        ...
        if(dataset != null) {
            ...
            String result = dbImpl.get5Id(dataset);
            result = result.isEmpty() ? "None" : result;
            resultMessage = AlexaConstants.ID_AVAILABLE + result;
        }
